...
# Turn the raw technical column names (and their category codes) into labels
# and help text a non-technical lender or field partner can actually understand.
FEATURE_LABELS = {
...
FEATURE_HELP = {
    ...
    "terms.loan_amount": "The amount the borrower requested, in US dollars.",
    "funded_amount": "How much lenders on Kiva actually raised for this loan, in US dollars.",
    "terms.disbursal_amount": (
        "The amount actually paid out to the borrower by the field partner, "
        "in the local currency (not USD)."
    ),
}
...
# Lets a user pick an explicit "unknown" option for categorical fields instead of
# being forced to guess a real category value they don't actually know.
...
def numeric_input_kwargs(feature, default):
    ...
    # Whole-number controls for fields where a fractional value would be
    # meaningless, like a year or a day-of-week.
    integer_features = {
        "partner_id",
        "posted_year",
        "posted_month",
...
# Reuses feature_label so importance tables show the same readable names as the input form.
...
# Check every required artifact exists before loading anything, so a missing file
# gives a clear message instead of a confusing crash later on.
...
# Load the already-fitted models and every supporting table produced by the notebook,
# so the app never retrains anything itself.
...
with tab_predict:
    ...
    with st.container(border=True):
        ...
        for feature in schema["feature_columns"]:
            ...
            # Country and town are linked so a user can't select a town that doesn't belong to the chosen country.
            ...
            with container:
                if feature == "borrower_gender_group":
                    ...
                    user_values[feature] = st.selectbox(
                        ...
                        # Tooltips added for the fields participants specifically asked to have explained:
                        # liability terms (P2) and the amount fields (P10).
    ...
    if predict_clicked:
        # Build a single-row dataframe matching the exact column order the model was trained on.
        ...
        # Most models give a probability directly; the SVD+Logistic model only has a
        # decision score, so convert that to a probability with a sigmoid instead.
...
with tab_importance:
    ...
    # Lets the viewer choose how many of the ~20 original features to see,
